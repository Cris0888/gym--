...
from claseRegistroProduct import Ventas
...
registro_productos =  Ventas (mysql) 
...
#--------------------------------REGISTRO DE VENTAS----------------------------------------------------------------------------#
@app.route("/Registro_venta", methods = ['GET'])
def registro_venta():
    if session.get("logueado"):
        resultado = registro_productos.consultarproduct()
        return render_template('dashboard/registro_ventas.html', resultado = resultado)
    else:
        return redirect('/')

    
@app.route('/Registro_venta/compra', methods=['POST'])
def procesar_compra():
    data = request.json
    productos_comprados = data['productos']
    # Aquí puedes hacer lo que desees con los productos comprados, como almacenarlos en una base de datos, etc.
    print("Productos comprados:", productos_comprados)
    return jsonify({"message": "Compra procesada correctamente"}), 200
