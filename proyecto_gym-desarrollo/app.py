...
from claseRegistroProduct import Registro
...
registro_productos = Registro(mysql) 
...
def registro_venta():
    if session.get("logueado"):
        return render_template('dashboard/registro_ventas.html')
    else:
        return redirect('/')
    
@app.route('/infoInventario' ,methods= ['GET'])
def infoInventario():
    resultado = InvProductos.consultarProductos()
